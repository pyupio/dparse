...
"""Tests for `dparse` package."""

import pytest

from dparse.parser import parse, RequirementsDependency, filetypes, DependencyFile
...
def test_update_tox_ini():
    ...
    assert dep.update(content, "2.9.5") == new_content
...
def test_update_conda_yml():
    ...
    assert dep.update(content, "4.5.6") == new_content
...
def test_update_requirements_multispace():
    ...
    dep_file = parse(content=content, filetype=filetypes.requirements_txt)
    dep = dep_file.dependencies[0]

    assert dep.name == "pass"
    assert dep.update(content=content, version=version) == new_content
...
def test_update_requirements_with_hashes():
    ...
    assert dep.update(content=content, version=version, hashes=hashes) == new_content
...
def test_update_requirements_line_endings():
    ...
    assert "Werkzeug==1.2.3\r\n" in dep_file.dependencies[0].update(content=content,
                                                                    version=version)
    assert "django-template-repl==1.2.3\n" in dep_file.dependencies[1].update(
        content=content, version=version)
    assert "bpython==1.2.3" in dep_file.dependencies[2].update(
        content=content, version=version)
    assert "some-fooo==1.2.3    \n" in dep_file.dependencies[3].update(
        content=content, version=version)
...
def test_index_server():
    ...
    assert DependencyFile.parse_index_server(line) == "https://some.foo/"
    ...
    line = "--index-url"
    assert DependencyFile.parse_index_server(line) == None
...
def test_file_resolver():
    ...
    dep_file = parse(content=content, path="/", filetype=filetypes.requirements_txt)
